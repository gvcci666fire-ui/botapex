...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('aggiungi-link')
        .setDescription('Aggiunge un link utile al pannello')
        .addStringOption((option) => option.setName('nome').setDescription('Nome del link').setRequired(true))
        .addStringOption((option) => option.setName('link').setDescription('URL del link').setRequired(true))
        .addStringOption((option) => option.setName('emoji').setDescription('Emoji da mostrare').setRequired(false))
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        store.links[name] = { emoji, url: link };
        writeStore(store);
        await refreshPanel(interaction, store);

        await interaction.reply({ content: `✅ Link aggiunto: ${name}`, flags: ['Ephemeral'] });
    }
};
